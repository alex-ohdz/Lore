...
from util import get_distr_values, calculate_feature_values, neuclidean, multilabel2str, multi_dt_predict, record2str
from sklearn.dummy import DummyClassifier
from rule import Rule, compact_premises, get_counterfactual_rules_supert, get_rule_supert, get_rule, get_counterfactual_rules
from explanation import Explanation, MultilabelExplanation
from neighgen import RandomGenerator, GeneticGenerator, RandomGeneticGenerator, ClosestInstancesGenerator, CFSGenerator, CounterGenerator, GeneticProbaGenerator, RandomGeneticProbaGenerator
from encdec import *
...
class LOREM(object):
    ...
    def __init__(self, K, bb_predict, predict_proba, feature_names, class_name, class_values, numeric_columns, features_map,
                 neigh_type='genetic', multi_label=False, K_transformed=None, categorical_use_prob=True, continuous_fun_estimation=False,
                 size=1000, ocr=0.1, one_vs_rest=False, filter_crules=True, init_ngb_fn=True,
        ...
        if init_ngb_fn:
            self.init_neighbor_fn(ocr, categorical_use_prob, continuous_fun_estimation, size, kwargs)
    ...
    # ?????????????????????????????????
    def init_neighbor_fn(self, ocr, categorical_use_prob, continuous_fun_estimation, size, kwargs):
    ...
    def explain_instance_stable(self, x, samples=100, use_weights=True, metric=neuclidean, runs=3, exemplar_num=5,
        ...
        else:            rule = get_rule_supert(x, superT, self.feature_names, self.class_name, self.class_values,
...
class TuClase:
    ...
    def explain_instance_stable_neigh(self, x, Z_list, use_weights=True, metric='euclidean', runs=3, exemplar_num=5,
